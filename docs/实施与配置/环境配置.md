...
## Profiles 优先级与覆盖（新增）
- 默认激活：`postgres`（当前约定），可通过 `SPRING_PROFILES_ACTIVE` 或启动参数覆盖。
- 覆盖顺序：`application.yml` < `application-{profile}.yml` < 环境变量/命令行参数。
- 切换示例（PowerShell）：
```powershell
$env:SPRING_PROFILES_ACTIVE="mysql"
mvn -f end/pom.xml spring-boot:run
```

## 健康检查与快速验证（新增）
1) 启动后端后检查日志包含：`Tomcat started on port 8080` 与激活的 `profile`。
2) 健康检查 URL（如启用 Actuator）：`GET /actuator/health` → `{ status: "UP" }`。
3) 基础 API 验证：
```powershell
Invoke-RestMethod -Uri "http://localhost:8080/api/v1/sections"
Invoke-RestMethod -Uri "http://localhost:8080/api/v1/threads?page=1&size=20"
```
4) 数据库连通性：在日志中查找连接字符串与 Flyway 迁移执行记录，无错误堆栈。
5) 常见问题速查：端口被占用/凭据不匹配/未加载正确的 `profile`。